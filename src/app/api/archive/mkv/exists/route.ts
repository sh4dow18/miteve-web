export async function GET(request: Request) {
  ...
  const candidates = [
    `${base}/archive/mkv/exists?filename=${encodeURIComponent(filename)}`,
    `${base}/archive/exists?filename=${encodeURIComponent(filename)}`,
    `${base.replace(":8084", ":8080")}/archive/mkv/exists?filename=${encodeURIComponent(filename)}`,
  ...
  return new Response(JSON.stringify({ detail: "No static resource archive/mkv/exists.", instance: "/archive/mkv/exists", status: 404, title: "Not Found" }), {
    status: 404,
