export async function GET(request: Request) {
  ...
  const candidates = [
    `${base}/archive/mp4/exists?filename=${encodeURIComponent(filename)}`,
    `${base}/archive/exists?filename=${encodeURIComponent(filename)}`,
    `${base.replace(":8084", ":8080")}/archive/mp4/exists?filename=${encodeURIComponent(filename)}`,
  ...
  return new Response(JSON.stringify({ detail: "No static resource archive/mp4/exists.", instance: "/archive/mp4/exists", status: 404, title: "Not Found" }), {
    status: 404,
