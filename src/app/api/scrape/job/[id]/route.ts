export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const candidates = [
    `${base}/scrape/job/${id}`,
    `${base.replace(":8084", ":8080")}/scrape/job/${id}`,
    `${base.replace("/new-api", "")}/scrape/job/${id}`,
  ];
  ...
  return new Response(JSON.stringify({ detail: "No static resource scrape/job.", instance: `/scrape/job/${id}`, status: 404, title: "Not Found" }), {
    status: 404,
