...
export async function POST(request: Request) {
  ...
  const { id, tmdbId, firstSeason, lastSeason, firstEpisode, lastEpisode } =
    BODY;
  ...
  const SEASON_RESPONSES = await Promise.all(
    SEASONS_TO_FETCH_LIST.map((seasonNumber) =>
      fetch(
        `https://api.themoviedb.org/3/tv/${tmdbId}/season/${seasonNumber}?api_key=${TMDB_API_KEY}&language=es-MX&append_to_response=videos,images`
      )
        .then(async (response) => {
  ...
  const SEASONS = await fetch(`${API_HOST_IP}/contents/${id}/seasons`, {
    method: "POST",
    headers: {
